...
class BoardWidget extends Pad  {
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (int i = 0; i <= SIDE; i++) {
            ...
            g.drawLine(0 + SQDIM * i, 0, 0 + SQDIM * i, _dim);
            g.drawLine(0, 0 + SQDIM * i, _dim,  0 + SQDIM * i);
        }
    ...
    void drawBlock(Graphics2D g, int cx, int cy) {
        ...
        int x = SQDIM / 2 + SQDIM * cx;
        int y = _dim - SQDIM / 2 - SQDIM * cy;

        if (_model.get(C[cx], R[cy]) == BLOCKED) {
            g.setStroke(BLOCK_STROKE);
            g.setColor(BLOCK_COLOR);
            g.drawLine(x - BLOCK_WIDTH / 2, y - BLOCK_WIDTH / 2,
            ...
            g.drawLine(x + BLOCK_WIDTH / 2, y + BLOCK_WIDTH / 2,
                    x - BLOCK_WIDTH / 2, y - BLOCK_WIDTH / 2);
        } else if (_model.get(C[cx], R[cy]) == RED) {
            g.setColor(RED_COLOR);
            g.fillOval(x, y, PIECE_RADIUS, PIECE_RADIUS);
        } else if (_model.get(C[cx], R[cy]) == BLUE) {
    ...
    private void handleClick(String unused, MouseEvent where) {
        ...
        if (where.getButton() == MouseEvent.BUTTON1) {
            ...
            if (mouseCol >= 'a' && mouseCol <= 'g'
                ...
                if (_blockMode) {
                    String command1 = String.format("block %c%c",
                            mouseCol, mouseRow);
                    _commandQueue.offer(command1);
                } else {
                    if (_selectedCol != 0) {
                        String command2 =
                                String.format("%c%c-%c%c", _selectedCol,
                                        _selectedRow, mouseCol, mouseRow);
                        _commandQueue.offer(command2);
                        _selectedCol = _selectedRow = 0;
                    } else {
